...
async function seed3MonthsData() {
  ...
  const products = await Promise.all([
    prisma.product.create({ data: { name: 'Apex Wireless ANC Headphones', sku: `APX-ANC-${Date.now().toString().slice(-4)}`, price: 6999, costPrice: 4200, stockQuantity: 120, reorderPoint: 25, unit: 'PCS', categoryId: catElectronics.id, tenantId: tenant.id } }),
    prisma.product.create({ data: { name: 'Apex Smartwatch Pro', sku: `APX-SW-${Date.now().toString().slice(-4)}`, price: 9999, costPrice: 6100, stockQuantity: 85, reorderPoint: 15, unit: 'PCS', categoryId: catElectronics.id, tenantId: tenant.id } }),
    prisma.product.create({ data: { name: 'Ergonomic Mesh Executive Chair', sku: `APX-CH-${Date.now().toString().slice(-4)}`, price: 14500, costPrice: 9200, stockQuantity: 40, reorderPoint: 10, unit: 'PCS', categoryId: catHome.id, tenantId: tenant.id } }),
    prisma.product.create({ data: { name: 'Organic Cotton Polo T-Shirt', sku: `APX-TS-${Date.now().toString().slice(-4)}`, price: 1499, costPrice: 650, stockQuantity: 250, reorderPoint: 50, unit: 'PCS', categoryId: catApparel.id, tenantId: tenant.id } }),
    prisma.product.create({ data: { name: 'UltraHD 4K Portable Monitor', sku: `APX-MON-${Date.now().toString().slice(-4)}`, price: 18999, costPrice: 13400, stockQuantity: 30, reorderPoint: 8, unit: 'PCS', categoryId: catElectronics.id, tenantId: tenant.id } }),
  ]);
  ...
  const now = new Date();
  const months = [
    { name: 'May 2026', daysAgoStart: 90, daysAgoEnd: 60, salesCount: 18, poCount: 3 },
    { name: 'June 2026', daysAgoStart: 60, daysAgoEnd: 30, salesCount: 28, poCount: 4 },
    { name: 'July 2026', daysAgoStart: 30, daysAgoEnd: 0, salesCount: 38, poCount: 5 },
  ];
